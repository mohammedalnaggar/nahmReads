...
login_button.addEventListener("click",(event)=>{
    ...
    let password=login_div.getElementsByTagName("input")[7].value;
...
function request_response(){
    ...
    xhttp.open("POST", "http://192.168.1.94:5000/users/login");
    xhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
    xhttp.send(JSON.stringify({
        email:signup_div.getElementsByTagName("input")[6].value,
        password:signup_div.getElementsByTagName("input")[7].value
    })
    )};
